...
function getIdByTitle(title: string, breedCollections: Record<string, { id: number; title: string; slug: string }>): string | null {
  for (const key in breedCollections) {
    if (breedCollections[key].title === title) {
      return JSON.stringify(breedCollections[key].id);
    }
  }
...
const CharacteristicPuppyForSell: React.FC<Props> = ({charater}) => {
  ...
  const [characterIDUrl, setCharacterIDUrl] = useState(getIdByTitle(charater.title, NavbarData.breedCollections)) 
  ...
  const [selectedCharacteristicsFilter, setSelectedCharacteristicsFilter] = useState<any[]>([
    getIdByTitle(charater.title, NavbarData.breedCollections)
  ])
  ...
  useEffect(() => {
    const isSlugPresent = selectedCharacteristicsFilter.some((characterID) => characterID === characterIDUrl);
  ...
  return (
    <div>
      <AuthenticationPopup />
      <CharacteristicPuppyForSellContainer 
